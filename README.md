...
Ah — fascinating! You're proposing a **computable infinite Wordle** where:
...
- After each guess, you get feedback for each digit: 🟩 (exact match), 🟨 (digit differs by 1 modulo base? or absolute difference = 1?), ⬛ (absolute difference $\ge 2$).
...
3. **Effect of base** — higher base makes ⬛ more common (since difference $\ge 2$ is easier), maybe easier to zero in?

4. **Can player ask for arbitrary positions each turn?** Yes — adaptive.

---

Would you like to **try designing a strategy** for base 4?

Or specify: Should “yellow” be **modular difference 1** (wrap-around: in base $b$, 0 and $b-1$ are neighbors), or **absolute difference 1** (so 0 neighbors only 1, $b-1$ neighbors only $b-2$)? This changes adjacency graph of digits.
